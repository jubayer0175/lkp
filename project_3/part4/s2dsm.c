...
/********Above this is the init protocol***********/
         ...
         char str[BUFF_SIZE] = "";
        int page_or_msi_temp,number_of_page_temp, number_of_page,\
            page_or_msi ;
        
        while(1){
            // wait to see if client send any request
            read(new_socket, &page_or_msi_temp, sizeof(int) );
            page_or_msi = ntohl(page_or_msi_temp);
            read(new_socket, &number_of_page_temp, sizeof(int));
            number_of_page = ntohl (number_of_page);
        //    printf("Request %d\n", page_or_msi);
            if(page_or_msi == 2){
...
/*		printf("\nfault_handler_thread():\n");
    ...
    //    memset(page, 'A' + fault_cnt % 20, page_size);
        ...
        t1 = htonl(1);
        int err;
        err = send(sock,&t1, sizeof(int), 0 );
        t1 = htonl(page_track);
        send(sock, &t1, sizeof(int), 0);
        printf("err: %d\n",err );
